...
export const UserPermissionListPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const response = await UserPermissionController.getEnhanced() as UserPermissionFormModel[];
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      await UserPermissionController.delete(dataToDelete.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">User Permissions Management</h1>
          <p className="text-muted-foreground">
            Manage and view all registered
          </p>
        </div>
        <Button onClick={() => navigate('/admin/user-permission/create')}>
            ...
            actions={(row) => (
              <div className="flex items-center space-x-2">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => navigate(`/admin/user-permission/edit/${row.id}`)}
                >
