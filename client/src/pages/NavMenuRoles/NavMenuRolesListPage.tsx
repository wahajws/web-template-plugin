...
import NavMenuRoleController from '@/controllers/admin/navMenuRoleController'; 
import { formatDate } from '@/utils/date';
import { Plus, Edit, Trash2 } from 'lucide-react';
import { getEnumLabel, RECORD_STATUS_OPTIONS } from '@constants/global';
import { TableColumn } from '@/models/common';

export const NavMenuRoleListPage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const result = await NavMenuRoleController.getAll(); // Change data controller here... 
      setData(result);
    } catch (error) {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      await NavMenuRoleController.delete(dataToDelete.id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Navigation Menu Permission Management</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Navigation Menu Permissions</CardTitle>
          <CardDescription>
